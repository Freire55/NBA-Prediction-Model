...
# 1. Load data and force strict chronological sorting to prevent data leakage
...
# Engineer HCA (Home Court Advantage) Decay based on the year
df['SEASON_YEAR'] = df['SEASON_ID'].astype(str).str[1:].astype(int)

# 2. Biological Schedule Density (Fatigue)
...
df['REST_DAYS'] = df['REST_DAYS'].fillna(5.0) # Assume 5 days of rest for the first game of the season
df['B2B'] = np.where(df['REST_DAYS'] == 1, 1, 0) # Back-to-Back flag

# Look back 2 and 3 games to find grueling schedule stretches (3-in-4 nights, 4-in-5 nights)
...
# Road Trip Length: Counts consecutive away games. Resets to 0 the moment they play at home.
...
# 3. Rolling Advanced Stats (Leak-Proof Method)
# Instead of fetching season-end advanced stats, we calculate rolling Offensive Rating using past games.
...
# We sum the points and possessions over the PREVIOUS 5 games using shift(1).
...
# Offensive Rating is (Points / Possessions) * 100
df['ROLLING_OFF_RATING'] = (df['ROLLING_PTS_5'] / df['ROLLING_POSS_5']) * 100
df['ROLLING_OFF_RATING'] = df['ROLLING_OFF_RATING'].fillna(100.0) # Baseline fill

# Standard 5-game rolling memory for all other Z-stats
...
# What was the rolling strength of the opponent we played?
...
# Average the strength of the last 5 opponents
...
# 5. Continuous Historical Elo Rating Simulation
print("Simulating historical Elo Ratings (Chronological Engine)...")
elo_dict = {}
elo_records = []

# Process games exactly as they happened in time
...
for index, row in unique_games.iterrows():
    ...
    # Store the PRE-GAME Elo. This prevents the model from looking into the future.
...
# 6. Merge into Matchups (Home vs Away)
...
# 7. Calculate Contextual Deltas (Home - Away)
